...
(A0_line,) = ax.plot(x, y, label="A0", color="#00549F") #00549F is the RWTH blue color
ax.set_ylabel("O2 [%]", color="#00549F")
...
(A2_line,) = ax2.plot(x, y3, label="A2", color="#CC071E") #00FFFF
ax2.set_ylabel("CO2 [%]", color="#CC071E")

#(A3_line,) = ax2.plot(x, y4, label="A3", color="#00FF00") #auf 2.y-Achse  00549F is the RWTH blue color
#ax2.set_ylabel("counts", color="#000000") #auf 2.y-Achse 

# plt.ylim(0, 150) commented out because it overrides dynamic axes scaling
plt.title("Bluesens: " + "    O2: " + str(A0) +" [%]  " + "   CO2: " +str(A2) + " [%]", fontsize=10) # To display 0 initially. Will be updated 
...
try:
    while True:     
        ...
        A0 = round((adc.read_adc(0, 1)*0.000125)*41.66666667-25,2)
        if A0 < 0:
            A0 = 0
        #A0 = round((adc.read_adc(0, 1)*0.000125*41.66666667-25),2)
        A1 = round((adc.read_adc(1, 1)/2),0)
        A2 = round((adc.read_adc(2, 1)*0.000125)*41.66666667-25,2)
        if A2 < 0:
            A2 = 0
        #A2 = round((adc.read_adc(2, 1)*0.000125*41.66666667-25),2)
        ...
        y4.append(A3)
        # ..update plot..
        plt.title("Bluesens: " + "    O2: " + str(A0) +" [%]  " + "   CO2: " +str(A2) + " [%]", fontsize=10)
